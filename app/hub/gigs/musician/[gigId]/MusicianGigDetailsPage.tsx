...
import PlatformActivitySidebar from "../_components/PlatformActivitySidebar";
...
// Add this component before your main component or in a separate file
const CancellationInfoModal = ({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      ...
      >
        {/* Decorative header - red for cancellations */}
        ...
        <div className="p-6">
          <DialogHeader className="space-y-2 mb-4">
            ...
            >
              You no longer have access to this gig's details
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                ...
                >
                  Cancellation{" "}
                  {info.cancelledBy === "client" ? "by Client" : "by System"}
                </p>
              </div>
              <p
                className={cn(
              ...
              >
                {info.reason || "No reason provided"}
              </p>
            </div>
            ...
            >
              <div className="flex items-center gap-2">
                ...
                >
                  Cancelled on{" "}
                  {new Date(info.cancelledAt).toLocaleDateString("en-US", {
                    weekday: "long",
                    year: "numeric",
                    month: "long",
                    day: "numeric",
            ...
            {/* Note about trust score */}
            {info.cancelledBy === "client" && (
              <div
                className={cn(
              ...
              >
                <p className="flex items-center gap-2">
                  ...
                  This cancellation may affect your trust score
                </p>
              </div>
            )}
          </div>
          ...
          <DialogFooter className="flex-col sm:flex-row gap-3 mt-6">
            ...
            <Button
              onClick={() => {
                ...
                router.push("/support?type=cancellation");
              }}
              className={cn(
                "w-full sm:w-auto rounded-xl",
...
export default function MusicianGigDetailsPage() {
  ...
  // In musician/[gigId]/gig-info/page.tsx and client/[gigId]/gig-info/page.tsx
  const poster = useQuery(
    api.controllers.user.getUserById,
    gig?.postedBy ? { userId: gig.postedBy as Id<"users"> } : "skip",
  );
  ...
  useEffect(() => {
    ...
    // Check if user has ANY relationship with this gig
    const hasAccess =
      gig.postedBy === currentUser._id ||
      gig.bookedBy === currentUser._id ||
      gig.interestedUsers?.includes(currentUser._id) ||
      gig.appliedUsers?.includes(currentUser._id) ||
      gig.shortlistedUsers?.some(
        (item: any) => item.userId === currentUser._id,
      ) ||
      gig.bandCategory?.some(
        (role: any) =>
          role.applicants?.includes(currentUser._id) ||
    ...
    // If user has no relationship, check if they were previously involved
    if (!hasAccess) {
      // Look for cancellation in booking history
      const cancellationEntry = gig.bookingHistory?.find(
        (entry: any) =>
      ...
      if (cancellationEntry) {
        // User was cancelled - show modal with details
        setCancellationInfo({
          gigTitle: gig.title,
          reason: cancellationEntry.reason || "No reason provided",
          cancelledBy:
            cancellationEntry.actionBy === gig.postedBy ? "client" : "system",
          cancelledAt: cancellationEntry.timestamp,
        });
        setShowCancellationModal(true);
      } else {
        // User never had access - just redirect
        router.push("/hub/gigs?tab=all");
        toast.info("You don't have access to this gig's details");
      }
    }
  }, [gig, currentUser, router]);
  ...
  return (
    <TooltipProvider>
      <CancellationInfoModal
        ...
        onRedirect={() => router.push("/hub/gigs?tab=all")}
      />
      <GigModal
        isOpen={showGigModal}
        onClose={() => {
          ...
          <div className="mb-8">
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                {gig.isTaken && (
                  <div className="px-3 py-1.5 rounded-lg text-xs font-medium bg-red-100  text-black  border border-red-200 dark:border-red-800">
                    Booked
                  </div>
                )}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-2 space-y-6">
              ...
              >
                ...
                >
                  <CardContent className="p-6 relative">
                    {/* BOOKED WATERMARK - EXTREME VERSION */}
                    {gig.isTaken && (
                      <div className="absolute inset-0 pointer-events-none">
                        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 rotate-[-30deg] text-[14rem] md:text-[16rem] lg:text-[14rem] font-black text-black/11 whitespace-nowrap leading-none tracking-tighter">
                          BOOKED
                        </div>
                      </div>
                    )}
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    {similarGigs.length > 0 && (
                      <Badge variant="outline" className="text-xs">
                        {Math.min(similarGigs.length, 2)} of{" "}
                        {similarGigs.length} shown
                      </Badge>
                    )}
                  </div>
                </div>
