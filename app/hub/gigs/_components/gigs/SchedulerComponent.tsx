...
const SchedulerComponent = ({
  ...
  // Add this function in your SchedulerComponent
  const validateRequiredFields = (): { isValid: boolean; errors: string[] } => {
    const errors: string[] = [];

    // Check basic requirements
    if (!isVerified) {
      errors.push("Account verification required");
    }
    ...
    if (activeOption === "regular" || activeOption === "automatic") {
      if (!isProUser && !isPremiumUser) {
        errors.push("Pro/Premium subscription required");
      }
    }
  ...
  const ConfirmationView = () => {
    ...
    // Validate whenever activeOption or relevant state changes
    useEffect(() => {
        ...
        <div className="flex-shrink-0 pt-4 mt-4 border-t border-gray-200 dark:border-gray-700">
          <div className="max-w-md mx-auto w-full px-2">
            <div className="flex flex-col gap-2">
              ...
              {/* ONLY SHOW LAUNCH BUTTON IF VALID */}
              {validation.isValid && (
                <motion.button
                  whileHover={{ scale: 1.01 }}
                  whileTap={{ scale: 0.99 }}
