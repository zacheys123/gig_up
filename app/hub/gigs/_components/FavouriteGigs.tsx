...
// Render grid view - Beautiful cards matching SavedGigs
          ...
          >
            <Card
              className={cn(
                ...
                isDarkMode
                  ? "bg-gray-900/50 border-gray-800 hover:border-yellow-600/50"
            ...
            >
              <CardContent className="p-3">
                ...
                <div className="mt-3 space-y-2">
                  ...
                  {dateStatus.isToday && (
                    <div className="mt-1">
                      <Badge className="w-full text-center text-[8px] py-0.5 bg-gradient-to-r from-green-500 to-emerald-500 text-white">
                        Happening Today! 🔥
                      </Badge>
                    </div>
                  )}
                ...
                <Button
                  ...
                  className={cn(
                    ...
                    isDarkMode
                      ? "border-gray-700 hover:border-yellow-600 hover:bg-yellow-600/10"
...
// Render list view - Matching SavedGigs list style
...
export const FavoriteGigs = ({ user }: { user: any }) => {
  ...
  // Stats - Mobile optimized with fewer stats on small screens
  const stats = useMemo(() => {
    if (!favoriteGigs?.length) return null;

    const active = favoriteGigs.filter(
      (g: any) => g.isActive && !g.isTaken,
    ).length;
    const upcoming = favoriteGigs.filter((g: any) => {
  ...
  const handleRemoveFavorite = async (gigId: string) => {
    try {
      await unfavoriteGig({ userId: user._id, gigId });
  ...
  if (isLoading) {
    return (
      <div className="space-y-4 p-2 sm:p-4">
        {/* Mobile stats skeleton - 3 columns on mobile */}
        <div className="grid grid-cols-3 gap-2 sm:hidden">
          {[...Array(3)].map((_, i) => (
            <Skeleton key={i} className="h-16 rounded-lg" />
          ))}
        </div>
        {/* Desktop stats skeleton - hidden on mobile */}
      ...
      <Card className="text-center py-12 sm:py-16 border-2 border-dashed mx-2 sm:mx-0">
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            >
              Star gigs you're interested in to see them here
            </p>
          </div>
          <Button
  ...
  return (
    <TooltipProvider>
      <div className="space-y-4 sm:space-y-6 p-2 sm:p-4">
        {/* Stats - Mobile optimized with gradients */}
        {stats && (
          <>
            {/* Mobile stats cards with gradients */}
            <div className="grid grid-cols-3 gap-2 sm:hidden">
              <Card className="text-center bg-gradient-to-br from-yellow-500/10 to-amber-500/10 border-0">
                <CardContent className="p-3">
                  <p
                    className={cn(
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {filteredGigs.length === 0 ? (
              ...
              renderGridView(filteredGigs, handleRemoveFavorite, isDarkMode)
            ) : (
              renderListView(filteredGigs, handleRemoveFavorite, isDarkMode)
            )}
          </motion.div>
        </AnimatePresence>
      </div>
