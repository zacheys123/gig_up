    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      {musicians.map((musician, index) => (
        ...
        >
          {/* 🔥 Top 3 Badge */}
          {index < 3 && (
            <div className="absolute -top-3 -right-3 bg-gradient-to-br from-amber-500 to-orange-500 text-white text-xs px-3 py-1 rounded-full font-semibold shadow-md">
              🔥 Top {index + 1}
            </div>
          )}
          ...
          <p className={cn("font-semibold text-lg text-center", colors.text)}>
            {musician.firstname || musician.username} {musician.lastname || ""}
          </p>
