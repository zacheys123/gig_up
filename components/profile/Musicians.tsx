...
const Musicians = ({ _id, isMobile }: MusiciansProps) => {
      ...
      >
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
          ...
          <button
            ...
            disabled={nearbyMusicians.length === 0}
            className="px-4 py-2 text-xs font-medium bg-gradient-to-br from-amber-500/20 to-purple-500/20 hover:from-amber-500/30 hover:to-purple-500/30 border border-amber-400/30 rounded-full text-amber-300 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {nearbyMusicians.length} nearby
          </button>
        </div>
          ...
          {filteredMusicians.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-gray-400">
                No musicians found in your network
              </p>
            </div>
          ) : (
...
// Extracted Musician Card Component
const MusicianCard = ({
  musician,
  onClick,
}: {
